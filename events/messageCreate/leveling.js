...
module.exports = async (polaris, message) => {
    try {
        ...
        if (guild.config.leveling.filter.includes(message.channel.id)) {
            return;
        }
        ...
        if (guild.config.leveling.type == 'static') {
            ...
            console.log((Math.floor(Math.random() * (parseInt(max) - parseInt(min) + 1)) + parseInt(min)) * multiplier);
        }
        ...
        if (member.stats.exp >= rankExp) {
            ...
            if (guild.config.leveling.rewards.status) {
                const rankRole = guild.config.leveling.rewards.rewards.find((reward) => reward.id == member.stats.rank);
